 ...
 * @date 2019-05-22
 *
 * @brief This file implements the functions from shared_memory.hpp that
 * encapsulate the use of the shared memory using the boost::interprocess
 * package. usage: see demos and unit tests and documentation
 */
                                     ...
                                     std::pair<ElemType*, std::size_t>& get_)
{
    ...
    if (objects_[object_id].second != get_.second)
    {
        ...
        throw shared_memory::Unexpected_size_exception(
            segment_id_, object_id, objects_[object_id].second, get_.second);
    }

    if (registered && VERBOSE)
    {
        std::cout << "shared_memory::SharedMemoryObject | registration of "
                  << object_id << " of size " << get_.second << std::endl;
    }
...
template <typename ElemType>
void SharedMemorySegment::set_object(
    ...
    const std::pair<const ElemType*, std::size_t>& set_)
{
    ...
    {
        std::cout << "shared_memory::SharedMemoryObject | registration of "
                  << object_id << " of size " << objects_[object_id].second
                  << std::endl;
    }
...
bool SharedMemorySegment::register_object(
    const std::string& object_id, const std::pair<ElemType*, std::size_t>& obj_)
{
    ...
    obj_ptr =
        segment_manager_
            .find_or_construct<typename std::remove_const<ElemType>::type>(
    ...
    const std::string& object_id)
{
    ...
    std::pair<typename std::remove_const<ElemType>::type*, std::size_t>
        obj_pair =
            segment_manager_.find<typename std::remove_const<ElemType>::type>(
                object_id.c_str());
         ...
         const std::map<KeyType, ValueType>& set_)
{
    ...
    {
        int i = 0;
        for (typename std::map<KeyType, ValueType>::const_iterator map_it =
                 set_.begin();
             map_it != set_.end();
             ++map_it)
        {
         ...
         bool create)
{
    ...
    {
        int i = 0;
        for (typename std::map<KeyType, ValueType>::iterator map_it =
                 get_.begin();
             map_it != get_.end();
             ++map_it)
        {
